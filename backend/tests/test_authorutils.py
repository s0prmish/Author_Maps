...
class TestAuthorNetwork:
    def test_get_publication_for_pair(self):
        cd = AuthorDataFactoryDummy  # classdummy
        t1 = "Test1"
        t2 = "Test2"
        t3 = "Test3"
        t4 = "Test4"
        ...
        # One intersection
        ret_12 = networkutils.get_publications_for_pair((t1, t2), AuthorDataFactory=AuthorDataFactoryDummy)
        assert type(ret_12) == list
        assert len(ret_12) == 1
        assert ret_12[0] == "C"

        # Two intersection
        ret_13 = networkutils.get_publications_for_pair((t1, t3), AuthorDataFactory=AuthorDataFactoryDummy)
        assert type(ret_13) == list
        assert len(ret_13) == 2
        assert "A" in ret_13 and "B" in ret_13

        # Three intersection
        ret_14 = networkutils.get_publications_for_pair((t1, t4), AuthorDataFactory=AuthorDataFactoryDummy)
        assert type(ret_14) == list
        assert len(ret_14) == 3
        assert "A" in ret_14 and "B" in ret_14 and "C" in ret_14

        # No intersection
        ret_15 = networkutils.get_publications_for_pair((t1, t5), AuthorDataFactory=AuthorDataFactoryDummy)
    ...
    def test_get_shared_publications_for_authors_pairs(self):
        ...
        t5 = "Test5"
        tl = [t1, t2, t3, t4, t5]
        test_dict = networkutils.get_shared_publications_for_authors_pairs(tl, AuthorDataFactory=cd)
        print(test_dict)
